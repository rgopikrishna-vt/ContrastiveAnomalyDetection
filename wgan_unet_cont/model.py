...
class Discriminator(nn.Module):
    ...
    def forward(self, input, sg_linear=False, projection1=False, projection2=False, penul = False):
        ...
        if projection1:
            _return_aux = True
            _aux['projection1'] = self.projection1(penultimate.view(penultimate.shape[0],-1))
        
        if projection2:
            _return_aux = True
            _aux['projection2'] = self.projection2(penultimate.view(penultimate.shape[0],-1))
